...
if [ "$CYGNUS_MONGO_HOSTS" != "" ]; then
    ...
    sed -i 's/'${CYGNUS_AGENT_NAME}'.sinks =/'${CYGNUS_AGENT_NAME}'.sinks = mongo-sink /g' ${FLUME_HOME}/conf/${AGENT_CONF_FILE}
    sed -i 's/'${CYGNUS_AGENT_NAME}'.channels =/'${CYGNUS_AGENT_NAME}'.channels = mongo-channel /g' ${FLUME_HOME}/conf/${AGENT_CONF_FILE}
