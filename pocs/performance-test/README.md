...
-  [負荷検証](#負荷検証)
    ...
    -  [負荷試験実施手順](#負荷試験実施手順)
        -  [1.事前準備](#1事前準備)
            -  [試験に使用するシェルの編集　【FIWAREサーバで作業を行う】](#試験に使用するシェルの編集FIWAREサーバで作業を行う)
            -  [Docker Composeファイルの決定　【作業対象なし】](#Docker-Composeファイルの決定作業対象なし)
            -  [負荷の条件の決定　【作業対象なし】](#負荷の条件の決定作業対象なし)
...
|測定時間|負荷が定常状態になってからの測定時間(秒)|86400|
|データ送信開始間隔|各デバイスがデータを送信開始する間隔(秒)|4|
|送信データタイプ|送信データの内容([string] or [number])|string|

※定常状態に達するまでの負荷を徐々に増加させられるように、各擬似デバイスがデータの送信を開始する間隔の指定を必須にしている。(データ送信開始間隔を0にすることで、負荷を急激に増加させることは可能)
