...
do
   docker run -d --net=host \
    ...
    -e MESSAGE_NUM=$(((${TEST_TIME}+${NUMBER_OF_CONTAINERS}*${STARTUP_TIME})/${SEND_MESSAGE_INTERVAL})) \
...
EXPECTED_FINISH_TIME=$((${NUMBER_OF_CONTAINERS}*${MESSAGE_SENDING_START_INTERVAL}*2+${TEST_TINE}+32400))
